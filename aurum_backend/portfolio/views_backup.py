...
from .models import Client, PortfolioSnapshot, Position, Transaction, Report
...
def generate_all_bond_issuer_weight_reports():
    ...
    try:
        ...
        # Get all clients with Fixed Income positions
        clients_with_bonds = Client.objects.filter(
            portfoliosnapshot__positions__asset__asset_type='Fixed Income'
        ).distinct()
        
        if not clients_with_bonds.exists():
            return Response({
                'success': False,
                'error': 'No clients with Fixed Income positions found'
...
def update_database(request):
    ...
    try:
        ...
        # Step 2: Calculate metrics for all clients
        calculation_service = PortfolioCalculationService()
        calculation_results = {}
...
def get_available_dates_by_type(request, report_type):
    ...
    report_type_mapping = {
        ...
        'bond_issuer_weight': 'BOND_ISSUER',
    ...
    available_dates = [date.strftime('%Y-%m-%d') for date in snapshot_dates
...
def admin_dashboard_data(request):
    """
    Enhanced admin dashboard with full metrics and chart data.
    Supports client filtering via ?client_code=ABC or ?client_code=ALL
    """
    try:
        # Get client filter parameter (support both 'client' and 'client_code' for compatibility)
        client_filter = request.GET.get('client_code') or request.GET.get('client', 'ALL')
        logger.info(f"Admin dashboard request with filter: {client_filter}")
...
def _generate_weighted_cumulative_return(clients_data, report_service):
    ...
    Uses Modified Dietz returns weighted by client AUM.
    """
    try:
        from collections import defaultdict
        all_client_returns = {}
        
        # Get cumulative return for each client
        for client_data in clients_data:
            client_code = client_data['client_code']
            client_weight = client_data['total_value']  # AUM as weight
            
            try:
                client = Client.objects.get(code=client_code)
                current_date = datetime.now().strftime('%Y-%m-%d')
                returns = report_service._generate_cumulative_return_chart(client, current_date)
                
                if returns['hasData']:
                    all_client_returns[client_code] = {
                        'data': returns['series'][0]['data'],
                        'weight': client_weight
                    }
            except Exception as e:
                logger.warning(f"Could not get returns for {client_code}: {e}")
                continue
        
        if not all_client_returns:
            return _get_empty_cumulative_return()
        
        # Aggregate by date with weighted returns
        date_aggregates = defaultdict(lambda: {'total_return': 0, 'total_weight': 0})
        
        for client_code, client_data in all_client_returns.items():
            weight = client_data['weight']
            for point in client_data['data']:
                date_key = point['x']  # timestamp
                # Convert back to return percentage: (value - 1000) / 1000 * 100
                return_pct = (point['y'] - 1000) / 1000 * 100
                date_aggregates[date_key]['total_return'] += return_pct * weight
                date_aggregates[date_key]['total_weight'] += weight
        
        # Calculate weighted average returns and convert back to base 1000
        aggregated_data = []
        for timestamp in sorted(date_aggregates.keys()):
            agg = date_aggregates[timestamp]
            if agg['total_weight'] > 0:
                weighted_return_pct = agg['total_return'] / agg['total_weight']
                # Convert back to base 1000: 1000 * (1 + return_pct/100)
                cumulative_value = 1000 * (1 + weighted_return_pct / 100)
                aggregated_data.append({'x': timestamp, 'y': round(cumulative_value, 2)})
        ...
        return {
            'hasData': True,
            'message': 'Weighted cumulative return',
            'series': [{'name': 'Weighted Cumulative Return (Base: 1000)', 'data': aggregated_data}],
            'currentValue': f"{aggregated_data[-1]['y']:.2f}",
            'currentDate': datetime.fromtimestamp(aggregated_data[-1]['x'] / 1000).strftime('%Y-%m-%d'),
            'yAxisMin': y_min,
            'yAxisMax': y_max,
...
def organize_uploaded_file(uploaded_file, bank_code, extracted_date):
    ...
    try:
        ...
        bank_destinations = {
            ...
            'IDB': base_processing_dir,  # Root directory
...
def scan_bank_files(bank_code, base_processing_dir, target_date=None):
    ...
    scan_locations = {
        ...
        'IDB': [base_processing_dir],  # Root directory only
    ...
    for location in locations:
        ...
        for file_path in bank_files:
            ...
            if 'securities' in filename_lower:
...
def get_bank_status(request):
    ...
    try:
        ...
        PROJECTAURUM_BANKS = [
            ...
            {'code': 'IDB', 'name': 'Inter-American Development Bank', 'type': 'simple'},
